...
export function PlayerPhotoUpload({ value, onChange }: { value: string; onChange: (url: string) => void }) {
  ...
  return <div className="flex items-center gap-3"><div className="grid h-16 w-16 overflow-hidden rounded-full border border-[var(--ea-gold)]/50 bg-black/20 place-items-center">{value ? <Image src={value} alt="Foto del jugador" width={64} height={64} unoptimized className="h-full w-full object-cover" /> : <Camera className="text-stone-500" />}</div><div><input ref={inputRef} className="hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => upload(event.target.files?.[0])} /><button type="button" disabled={uploading} onClick={() => inputRef.current?.click()} className="rounded-md border border-[var(--ea-gold)]/60 px-3 py-2 text-sm font-semibold text-[var(--ea-gold)] disabled:opacity-50">{uploading ? "Cargando..." : "Subir foto"}</button><p className="mt-1 text-xs text-stone-500">JPG, PNG o WebP · hasta 2 MB</p></div></div>;
}
