...
export function PlayerAssignmentDialog({ open, onOpenChange, player, selectablePlayers = [], teamRegistrations, fixedTeamId, loading, onSave }: Props) {
  const { register, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm<PlayerAssignmentValues>({ resolver: zodResolver(playerAssignmentSchema), defaultValues: { player_id: "", team_registration_id: "", shirt_number: 1, is_captain: false, is_goalkeeper: false } });
