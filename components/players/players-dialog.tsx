...
type PlayerDialogProps = { open: boolean; onOpenChange: (open: boolean) => void; player: Player | null; teamRegistrations: TeamRegistrationOption[]; loading: boolean; onSave: (values: PlayerCreateValues) => Promise<void>; fixedTeamId?: string };

export function PlayerDialog(props: PlayerDialogProps) {
  return <PlayerDialogForm key={`${props.open}:${props.player?.id ?? ""}:${props.fixedTeamId ?? ""}`} {...props} />;
}

function PlayerDialogForm({ open, onOpenChange, player, teamRegistrations, loading, onSave, fixedTeamId }: PlayerDialogProps) {
  const defaultValues = player ? { ...emptyValues, document_type: "DNI" as const, document_number: player.document_number, first_name: player.first_name, last_name: player.last_name, birth_date: player.birth_date ?? "", photo_url: player.photo_url ?? "" } : emptyValues;
  const { register, handleSubmit, setValue, control, formState: { errors } } = useForm<PlayerCreateValues>({ resolver: zodResolver(playerCreateSchema), defaultValues });
