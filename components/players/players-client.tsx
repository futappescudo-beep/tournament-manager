...
export function PlayersClient({ players: initialPlayers, teamRegistrations }: { players: Player[]; teamRegistrations: TeamRegistrationOption[] }) {
  ...
  async function remove(player: Player) {
    if (!window.confirm(`¿Quitar a ${player.first_name} ${player.last_name} de sus equipos activos? Su ficha seguirá disponible en el historial de jugadores.`)) return;
    setLoading(true);
    try {
      await deletePlayer(player.id);
      setPlayers((current) => current.map((item) => item.id === player.id ? { ...item, assignments: [] } : item));
