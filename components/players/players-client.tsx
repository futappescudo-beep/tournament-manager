...
import type { PlayerAssignmentValues, PlayerFormValues } from "@/lib/validations/players";
...
export function PlayersClient({ players: initialPlayers, teamRegistrations }: { players: Player[]; teamRegistrations: TeamRegistrationOption[] }) {
  ...
  async function save(values: PlayerFormValues) {
    setLoading(true);
    try {
      if (selected) {
        await updatePlayer(selected.id, values);
        setPlayers((current) => current.map((player) => player.id === selected.id ? { ...player, ...values, birth_date: values.birth_date || null } : player));
      } else {
        const created = await createPlayer(values);
        setPlayers((current) => [...current, { ...(created as Player), current_assignment: null }].sort((a, b) => a.last_name.localeCompare(b.last_name)));
      }
      setOpen(false);
      setSelected(null);
  ...
  async function saveAssignment(values: PlayerAssignmentValues) {
    ...
    try {
      ...
      setPlayers((current) => current.map((player) => player.id === values.player_id ? { ...player, current_assignment: { id: "new", team_registration_id: values.team_registration_id, shirt_number: values.shirt_number, is_captain: values.is_captain, is_goalkeeper: values.is_goalkeeper, label } } : player));
