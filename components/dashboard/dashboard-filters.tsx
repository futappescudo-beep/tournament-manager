...
export function DashboardFilters({ catalog, filter, path = "/dashboard", idPrefix = "dashboard" }: Props) {
  ...
  function navigate(next: DashboardFilter) {
    ...
    startTransition(() => router.push(`${path}${params.size ? `?${params.toString()}` : ""}`));
  }

  return <div className="ea-panel grid gap-3 rounded-lg p-4 md:grid-cols-3">
    <label className="sr-only" htmlFor={`${idPrefix}-tournament`}>Torneo</label>
    <select id={`${idPrefix}-tournament`} value={tournamentId} onChange={(event) => {
    ...
    <label className="sr-only" htmlFor={`${idPrefix}-category`}>Categoría</label>
    <select id={`${idPrefix}-category`} disabled={!tournamentId} value={categoryId} onChange={(event) => {
      const value = event.target.value;
    ...
    <label className="sr-only" htmlFor={`${idPrefix}-zone`}>Zona</label>
    <select id={`${idPrefix}-zone`} disabled={!categoryId} value={zoneId} onChange={(event) => {
      const value = event.target.value;
      setZoneId(value);
