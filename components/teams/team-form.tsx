...
export default function TeamForm({ team, categories, loading = false, onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-3"><h2 className="text-lg font-semibold">Inscripciones</h2><p className="text-sm text-muted-foreground">Selecciona una o más combinaciones de categoría y zona.</p>
        ...
        {categories.map((category) => <div key={category.id} className="rounded-lg border p-4"><h3 className="font-medium">{category.name}</h3>{category.zones.length ? <div className="mt-3 flex flex-wrap gap-3">{category.zones.map((zone) => { const checked = registrations.some((registration) => registration.category_id === category.id && registration.zone_id === zone.id); return <label key={zone.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={checked} onChange={() => toggleRegistration(category.id, zone.id)} />{zone.name}</label>; })}</div> : <p className="mt-2 text-sm text-amber-300">Esta categoría aún no tiene zonas.</p>}</div>)}
