...
export function ResultsClient({ matches, canEdit = false }: { matches: FixtureMatch[]; canEdit?: boolean }) {
  ...
  return <div className="space-y-6">
    <div><p className="text-xs font-bold tracking-[.18em] text-[var(--ea-gold)]">COMPETENCIA</p><h1 className="mt-1 text-3xl text-white">Resultados</h1><p className="mt-1 text-sm text-stone-400">{canEdit ? "Cargá o corregí los marcadores de cada partido." : "Consultá los marcadores registrados de cada partido."}</p></div>
    <section className="ea-panel overflow-hidden rounded-lg">
      ...
      {matches.length ? <div className="divide-y divide-[var(--ea-border)]">{matches.map((match) => <article key={match.id} className="grid gap-3 p-5 sm:grid-cols-[1fr_auto_1fr_auto] sm:items-center">
        ...
        {canEdit && <button disabled={match.sheetStatus === "CLOSED"} onClick={() => openResult(match)} className="inline-flex items-center justify-center gap-2 rounded-md border border-[var(--ea-gold)]/40 px-3 py-2 text-sm font-semibold text-[var(--ea-gold-soft)] hover:bg-white/5 disabled:cursor-not-allowed disabled:border-stone-700 disabled:text-stone-500"><Pencil size={15} />{match.sheetStatus === "CLOSED" ? "Planilla cerrada" : match.home_score === null || match.away_score === null ? "Cargar" : "Editar"}</button>}
