...
export async function getDashboardCatalog(): Promise<DashboardCatalog> {
  ...
  const [tournaments, categories, zones] = await Promise.all([supabase.from("tournaments").select("id,name").is("deleted_at", null).is("archived_at", null).order("name"), supabase.from("categories").select("id,tournament_id,name").is("deleted_at", null).eq("active", true).order("display_order"), supabase.from("zones").select("id,category_id,name").is("deleted_at", null).order("display_order")]);
  ...
  const activeTournamentIds = new Set((tournaments.data ?? []).map((tournament) => tournament.id));
  const visibleCategories = (categories.data ?? []).filter((category, index, all) => activeTournamentIds.has(category.tournament_id) && all.findIndex((item) => item.tournament_id === category.tournament_id && catalogKey(item.name) === catalogKey(category.name)) === index);
...
export async function getDashboardData(filter: DashboardFilter, standingsLimit = 5): Promise<{ teamCount: number; playerCount: number; goalCount: number; matches: FixtureMatch[]; standings: Standing[] }> {
  ...
  const activeCategories = (rawCategories ?? []).filter((category) => activeTournamentIds.has(category.tournament_id));
  const selectedCategory = catalog.categories.find((category) => category.id === filter.categoryId);
  const categoryIds = selectedCategory
    ? activeCategories.filter((category) => category.tournament_id === selectedCategory.tournament_id && catalogKey(category.name) === catalogKey(selectedCategory.name)).map((category) => category.id)
    : filter.tournamentId
