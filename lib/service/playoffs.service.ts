...
export async function getPlayoffBrackets(): Promise<PlayoffBracketSummary[]> {
  ...
  const { data: activeTournaments, error: tournamentsError } = await supabase.from("tournaments").select("id").in("id", [...new Set(brackets.map((bracket) => bracket.tournament_id))]).is("deleted_at", null).is("archived_at", null);
