...
import { GuestCompetitionFilters } from "@/components/public/guest-competition-filters";
import { getPublicCompetitionCatalog, getPublicFixture, getPublicStandings, getPublicSuspensions, getPublicTopScorers, goalDifference, points } from "@/lib/service/competition.service";
...
export default async function PublicCompetitionPage({ searchParams }: { searchParams: Promise<{ view?: string; tournament?: string; category?: string; zone?: string }> }) {
  const params = await searchParams;
  const view: GuestView = isGuestView(params.view) ? params.view : "principal";
  const [catalogResult, fixtureResult, standingsResult, scorersResult, suspensionsResult] = await Promise.allSettled([getPublicCompetitionCatalog(), getPublicFixture(), getPublicStandings(), getPublicTopScorers(), getPublicSuspensions()]);
  const catalog = catalogResult.status === "fulfilled" ? catalogResult.value : { tournaments: [], categories: [], zones: [] };
  ...
  const hasLoadError = [catalogResult, fixtureResult, standingsResult, scorersResult, suspensionsResult].some((result) => result.status === "rejected");
  const tournamentId = catalog.tournaments.some((item) => item.id === params.tournament) ? params.tournament : undefined;
  const categoryId = tournamentId && catalog.categories.some((item) => item.id === params.category && item.tournamentId === tournamentId) ? params.category : undefined;
  const zoneId = categoryId && catalog.zones.some((item) => item.id === params.zone && item.categoryId === categoryId) ? params.zone : undefined;
  const scope = (id: string | null | undefined, expected: string | undefined) => !expected || id === expected;
  const scopedFixture = fixture.filter((match) => scope(match.tournamentId, tournamentId) && scope(match.categoryId, categoryId) && scope(match.zoneId, zoneId));
  const scopedStandings = standings.filter((standing) => scope(standing.tournamentId, tournamentId) && scope(standing.categoryId, categoryId) && scope(standing.zoneId, zoneId));
  const scheduledMatches = scopedFixture.filter((match) => Boolean(match.match_date || match.kickoff_time));
  const results = scopedFixture.filter((match) => match.home_score !== null || match.away_score !== null);
  const filterQuery = new URLSearchParams({ ...(tournamentId ? { tournament: tournamentId } : {}), ...(categoryId ? { category: categoryId } : {}), ...(zoneId ? { zone: zoneId } : {}) }).toString();
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_-10%,rgba(100,8,11,.28),transparent_26rem),#090a0a] text-foreground lg:grid lg:grid-cols-[16rem_1fr]">
    <aside className="border-b border-[var(--ea-border)] bg-[linear-gradient(180deg,#0b0c0c,#11100f)] lg:min-h-screen lg:border-b-0 lg:border-r">
      ...
      <nav className="flex gap-1 overflow-x-auto p-3 lg:block lg:space-y-1">{guestViews.map(({ key, label, icon: Icon }) => <Link key={key} href={`/public?view=${key}${filterQuery ? `&${filterQuery}` : ""}`} className={`inline-flex shrink-0 items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium transition lg:flex ${view === key ? "bg-[#a70e15] text-white" : "text-stone-300 hover:bg-white/5 hover:text-white"}`}><Icon size={18} />{label}</Link>)}</nav>
    </aside>
    <section className="min-w-0">
      ...
      <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
        ...
        {hasLoadError && <p className="rounded-md border border-amber-500/40 bg-amber-500/10 p-4 text-sm text-amber-100">La información pública todavía se está habilitando. Ejecutá la migración <code>20260929_guest_competition_filters.sql</code> en Supabase y recargá esta página.</p>}
        <GuestCompetitionFilters catalog={catalog} view={view} tournamentId={tournamentId} categoryId={categoryId} zoneId={zoneId} />
        ...
        {(view === "principal" || view === "posiciones") && <StandingsPanel standings={scopedStandings} />}
