...
import { canManageFixture, getAccessibleMatchSheetIds, getFixture, getFixturePhases, getFixtureSetup, type FixtureMatch } from "@/lib/service/competition.service";
...
function MatchRow({ match, isManager, setup, canOpenSheet }: { match: FixtureMatch; isManager: boolean; setup: Setup; canOpenSheet: boolean }) {
  return <article className="grid gap-3 p-5 lg:grid-cols-[9rem_1fr_auto_1fr_12rem]"><div className="text-sm"><p className="font-semibold text-[var(--ea-gold-soft)]">{match.phaseName ?? "Fase"} · Fecha {match.round ?? "-"}</p><p className="mt-1 text-stone-400">{match.match_date ? new Date(`${match.match_date}T00:00:00`).toLocaleDateString("es-AR") : "Pendiente de programación"}</p><p className="text-stone-500">{match.kickoff_time?.slice(0, 5) ?? "--:--"}</p></div><strong className="self-center text-right text-base">{match.home_team ?? "Por definir"}</strong><span className="self-center rounded border border-[var(--ea-gold)]/30 px-3 py-1 text-center font-bold text-[var(--ea-gold)]">{match.home_score ?? 0} - {match.away_score ?? 0}</span><strong className="self-center text-base">{match.away_team ?? "Por definir"}</strong><div className="self-center text-xs text-stone-500"><p className="flex gap-1"><MapPin size={14} />{match.field ?? "Cancha a definir"}</p><p className="mt-1 flex gap-1"><UserRound size={14} />{match.referee ?? "Sin árbitro"}</p>{match.supervisor && <p className="mt-1 flex gap-1"><UserRound size={14} />Veedor: {match.supervisor}</p>}{isManager && setup && <FixtureScheduleEditor match={match} setup={setup} />}{canOpenSheet && <Link href={`/matches/${match.id}`} className="mt-3 block font-semibold text-[var(--ea-gold)] hover:text-[var(--ea-gold-soft)]">Planilla y eventos</Link>}</div></article>;
}
...
export default async function MatchesPage({ searchParams }: { searchParams: Promise<FixtureSearchParams> }) {
  ...
  const [matches, catalog, phases, isManager, brackets, accessibleSheetIds] = await Promise.all([getFixture(), getDashboardCatalog(), getFixturePhases(), canManageFixture(), getPlayoffBrackets(), getAccessibleMatchSheetIds()]);
...
function Agenda({ matches, isManager, setup, accessibleSheetIds }: { matches: FixtureMatch[]; isManager: boolean; setup: Setup; accessibleSheetIds: Set<string> | null }) { return <FixtureList title="Partidos programados" description="Todos los encuentros con día y hora asignados, sin importar torneo, categoría o zona." matches={matches} isManager={isManager} setup={setup} accessibleSheetIds={accessibleSheetIds} empty="Todavía no hay partidos con día y hora programados." />; }
function FixtureList({ title, description, matches, isManager, setup, accessibleSheetIds, empty = "No hay partidos para los filtros seleccionados." }: { title: string; description: string; matches: FixtureMatch[]; isManager: boolean; setup: Setup; accessibleSheetIds: Set<string> | null; empty?: string }) { return <section className="ea-panel overflow-hidden rounded-lg"><div className="flex items-center justify-between border-b border-[var(--ea-border)] p-5"><div><h2 className="ea-heading text-xl">{title}</h2><p className="mt-1 text-sm text-stone-400">{description}</p></div><span className="rounded bg-[#2b2010] px-3 py-1 text-sm text-[var(--ea-gold)]">{matches.length} encuentros</span></div>{matches.length ? <div className="divide-y divide-[var(--ea-border)]">{matches.map((match) => <MatchRow key={match.id} match={match} isManager={isManager} setup={setup} canOpenSheet={accessibleSheetIds === null || accessibleSheetIds.has(match.id)} />)}</div> : <Empty text={empty} />}</section>; }
