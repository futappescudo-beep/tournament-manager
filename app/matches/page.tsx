...
function MatchRow({ match, isManager, setup }: { match: FixtureMatch; isManager: boolean; setup: Setup }) {
  return <article className="grid gap-3 p-5 lg:grid-cols-[9rem_1fr_auto_1fr_12rem]"><div className="text-sm"><p className="font-semibold text-[var(--ea-gold-soft)]">{match.phaseName ?? "Fase"} · Fecha {match.round ?? "-"}</p><p className="mt-1 text-stone-400">{match.match_date ? new Date(`${match.match_date}T00:00:00`).toLocaleDateString("es-AR") : "Pendiente de programación"}</p><p className="text-stone-500">{match.kickoff_time?.slice(0, 5) ?? "--:--"}</p></div><strong className="self-center text-right text-base">{match.home_team ?? "Por definir"}</strong><span className="self-center rounded border border-[var(--ea-gold)]/30 px-3 py-1 text-center font-bold text-[var(--ea-gold)]">{match.home_score ?? 0} - {match.away_score ?? 0}</span><strong className="self-center text-base">{match.away_team ?? "Por definir"}</strong><div className="self-center text-xs text-stone-500"><p className="flex gap-1"><MapPin size={14} />{match.field ?? "Cancha a definir"}</p><p className="mt-1 flex gap-1"><UserRound size={14} />{match.referee ?? "Sin árbitro"}</p>{match.supervisor && <p className="mt-1 flex gap-1"><UserRound size={14} />Veedor: {match.supervisor}</p>}{isManager && setup && <FixtureScheduleEditor match={match} setup={setup} />}<Link href={`/matches/${match.id}`} className="mt-3 block font-semibold text-[var(--ea-gold)] hover:text-[var(--ea-gold-soft)]">Planilla y eventos</Link></div></article>;
}
