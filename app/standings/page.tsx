...
export default async function StandingsPage({ searchParams }: { searchParams: Promise<{ tournament?: string; category?: string; zone?: string }> }) {
  ...
  const hasCompleteScope = Boolean(tournamentId && categoryId && zoneId);
  const { standings } = hasCompleteScope ? await getDashboardData(filter, 0) : { standings: [] };
