# Requisitos funcionales y estado

| ID | Requisito | Estado |
| --- | --- | --- |
| RF-001 | Crear y administrar torneo, categoría, zonas y cupos | Implementado |
| RF-002 | Crear, editar y dar de baja equipos | Implementado |
| RF-003 | Registrar jugador y asignarlo por equipo/categoría/zona | Implementado |
| RF-004 | Programar fixture manual | Implementado |
| RF-005 | Cargar resultados y recalcular tabla | Implementado |
| RF-006 | Registrar goles, tarjetas y consultar sanciones | Implementado |
| RF-007 | Administrar canchas, árbitros, asistentes y veedor | Implementado; veedor requiere migración 20260923 |
| RF-008 | Consultar información de competencia sin iniciar sesión | Implementado: navegación pública de lectura, fixture, resultados, posiciones, goleadores, sanciones, reglamento y filtros Torneo → Categoría → Zona |
| RF-009 | Subir planilla PDF o imagen | Pendiente |
| RF-010 | Generar fixture automático regular por zona y programarlo después | Implementado; requiere migración 20260919 |
| RF-011 | Diseñar playoffs flexibles, publicar cruces y avanzar ganadores | Implementado; requiere migraciones 20260918 y 20260921 |
| RF-012 | Pre-cargar, abrir, cerrar y cancelar una planilla digital | Implementado; requiere migraciones 20260916, 20260917 y 20260922 |
| RF-013 | Archivar torneo y excluirlo de la operación activa | Implementado; requiere migración 20260920 |
| RF-014 | Generar fixture interzonal opcional | Pendiente |
| RF-015 | Administrar pagos (ABM, estados y comprobantes) | Parcial: solo consulta |
| RF-016 | Consultar historial de torneos, campeones y ranking de títulos | Pendiente |
| RF-017 | Aplicar permisos por equipo delegado y partido arbitral | Parcial: jugador en modo consulta y delegado único por equipo con acceso de lectura/confirmación a sus planillas. La cuenta arbitral compartida es una decisión operativa; falta su aceptación y trazabilidad del oficial físico. |
| RF-018 | Instalar la aplicación y enviar notificaciones | Pendiente: PWA y push |
