...
Una `Matchday` pertenece a torneo, categoría, zona y fase. Un `Match` enlaza jornada, inscripciones local/visitante, recursos (cancha, árbitro, asistentes y veedor), estado y marcador. `MatchEvent` genera estadísticas; las vistas `vw_fixture`, `vw_standings` y `vw_top_scorers` son proyecciones de lectura.
